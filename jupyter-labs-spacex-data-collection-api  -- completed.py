...
if response.status_code == 200:
    ...
    # Use json_normalize to convert the json result into a dataframe
...
# my code as alternatve to previous one
import pandas as pd
import requests
import datetime

# Example of fetching data, ensure this matches your actual method of getting data
...
# Extract the single value from the cores and payloads lists
data['cores'] = data['cores'].apply(lambda x: x[0])
data['payloads'] = data['payloads'].apply(lambda x: x[0])

# Convert 'date_utc' to date and filter dates
data['date'] = pd.to_datetime(data['date_utc']).dt.date
data = data[data['date'] <= datetime.date(2020, 11, 13)]
...
print(launch_data.describe())

# Check for any null values
print(launch_data.isnull().sum())
...
# Assume 'launch_data' is the name of your DataFrame
# Display the first few rows of the DataFrame
print(launch_data.head())

# If you want to see, for example, the first 10 rows instead of just 5
print(launch_data.head(10))
...
# Assuming 'launch_data' is your original DataFrame
# Filter the DataFrame to only include Falcon 9 launches
data_falcon9 = launch_data[launch_data['BoosterVersion'] == 'Falcon 9']

# Display the head of the new DataFrame to verify the filter
...
# Get counts of each unique value in the BoosterVersion column
booster_counts = data_falcon9['BoosterVersion'].value_counts()
...
# Additional step: Get the list of rockets to filter by rocket type
...
falcon_9_launches = [launch for launch in data if rocket_id_to_name[launch['rocket']] == 'Falcon 9']
...
print(f"falcon_9_count: {falcon_9_count}")
